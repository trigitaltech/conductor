...
export default function WorkflowDefinition() {
  ...
  const dag = useMemo(() => workflow && new WorkflowDAG(null, workflow), [workflow])
  ...
  return (
    <div className={classes.wrapper}>
      ...
      <div className={classes.header} style={{ paddingBottom: 20 }}>
        ...
        <Heading level={4} gutterBottom>{match.params.name}</Heading>

        <Select
          value={_.isUndefined(version) ? "": version}
          displayEmpty
          renderValue={v => v === "" ? "Latest Version": v}
          onChange={(evt) => history.push(`/workflowDef/${workflowName}${evt.target.value === ""? "": "/"}${evt.target.value}`)}
        >
          <MenuItem value="">Latest Version</MenuItem>
          {versions.map(ver => <MenuItem value={ver} key={ver}>Version {ver}</MenuItem>)}
        </Select>
      </div>
