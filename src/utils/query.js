...
export function useFetch(path, reactQueryOptions) {
  ...
  return useQuery([fetchContext.stack, path], () => fetchWithContext(path, fetchContext), {
...
export function useWorkflowSearch(searchObj) {
  ...
  return useQuery([fetchContext.stack, pathRoot, searchObj], () => {
      
      const { rowsPerPage, page, sort, freeText, query} = searchObj;
      const path = pathRoot + qs.stringify({
...
export function useTaskQueueInfo(taskName){
  ...
  const {data: pollData, isFetching: pollDataFetching} = useQuery([fetchContext.stack, pollDataPath], () => fetchWithContext(pollDataPath, fetchContext), {
    enabled: fetchContext.ready && !_.isEmpty(taskName),
  });
  const {data: size, isFetching: sizeFetching} = useQuery([fetchContext.stack, sizePath], () => fetchWithContext(sizePath, fetchContext), {
    enabled: fetchContext.ready && !_.isEmpty(taskName),
  });

  const taskQueueInfo = useMemo(() => ({size: _.get(size, [taskName]), pollData:pollData}), [taskName, pollData, size]);
...
export function useAction(path, method="post", callbacks){
  const fetchContext = useFetchContext();
  return useMutation((mutateParams) => fetchWithContext(path, fetchContext, { 
    method, 
    headers: {
...
export function useWorkflowDefs () {
  const {data, ...rest } = useFetch('/metadata/workflow', {staleTime: STALE_TIME_WORKFLOW_DEFS });
    ...
    if(data){
      ...
      for (let workflowDef of data) {
        if (!unique.has(workflowDef.name)) {
          ...
          unique.get(workflowDef.name).version < workflowDef.version
        ) {
          unique.set(workflowDef.name, workflowDef);
        }
...
export function useWorkflowNamesAndVersions() {
  const {data } = useFetch('/metadata/workflow', {staleTime: STALE_TIME_WORKFLOW_DEFS });
...
export function useTaskNames() {
  const {data} = useFetch(`/metadata/taskdefs`, {staleTime: STALE_TIME_DROPDOWN});
  return useMemo(() => data ? Array.from(new Set(data.map((def) => def.name))).sort(): [], [data]);
}
