...
import { Paper, NavLink, DataTable, LinearProgress, TertiaryButton, Text } from "../../components";
...
const executionFields = [
  ...
  { name: "workflowId", grow: 2, renderer: workflowId => <NavLink path={`/execution/${workflowId}`}>{workflowId}</NavLink>},
...
function ShowMore ({ rowsPerPage, rowCount, onChangePage, onChangeRowsPerPage, currentPage }){
  return <div style={{textAlign: 'center', padding: 15}}>
    <TertiaryButton onClick={() => onChangePage(currentPage+1)}>Show More Results</TertiaryButton>
  </div>
};
...
export default function ResultsTable({
  ...
  const totalHits = resultObj? (resultObj.totalHits === undefined? resultObj.results.length: resultObj.totalHits) : 0;
    ...
    <Paper className={classes.paper}>
      ...
      {resultObj && (
        <DataTable
          ...
          selectableRows
          contextComponent={<BulkActionModule selectedRows={selectedRows} popperAnchorEl={tableRef.current}/>}
