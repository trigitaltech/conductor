...
export default function DataTable (props) {
  ...
  const [tableState, setTableState] = useLocalStorage(localStorageKey, defaultColumns);

  const [filterObj, setFilterObj] = useState(initialFilterObj || DEFAULT_FILTER_OBJ);
  ...
  const dataTableColumns = useMemo(() => {
    ...
    return viewColumns.map((column) => {
      ...
      if (renderer) {
        internalOptions.format = (row) => renderer(_.get(row, name));
      }
  ...
  const filteredItems = useMemo(() => {
    const column = dataTableColumns.find(col => col.id === filterObj.columnName);
    ...
    if (!filterObj.substring || !filterObj.columnName) {
      ...
      return data.filter((row) => {
        let target;
          if(column.type === 'json' || column.type === 'date' || column.searchable === 'calculated'){
    ...
    <RawDataTable
      ...
      actions={
        <>
          ...
          { showColumnSelector && <ColumnsSelector
            columns={columns}
            selected={tableState}
  ...
  return (
    <>
      <Tooltip title="Show Columns">
        <IconButton onClick={handleClick} label="Columns" color={(_.get(filterObj, "substring") !== "" )? "primary": "default"}>
          <SearchIcon />
        </IconButton>
      </Tooltip>
      <Popover
  ...
  return (
    <>
      ...
      >
        {[
          ...columns.map((column) => (
            <MenuItem key={getColumnId(column)} value={getColumnId(column)} dense>
              <Checkbox
                checked={selected.includes(getColumnId(column))}
                onChange={(e) => handleChange(getColumnId(column), e.target.checked)}
              />
